...
import { Globe, Settings, Cloud, Zap, Shield, Smartphone, CheckCircle, ArrowRight, Banknote, ShieldCheck, TrendingUp, DollarSign, Home, BarChart2 } from "lucide-react";
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-slate-900 mb-3">Financial Products</h2>
            <p className="text-slate-500">Comprehensive financial solutions to grow and protect your wealth.</p>
          </div>
          <motion.div
            variants={staggerContainer}
            initial="hidden"
          ...
          >
            {financeServices.map((f) => (
              <motion.div variants={fadeIn} key={f.title} className="card card-hover">
                <div className={`w-11 h-11 ${f.iconBg} rounded-xl flex items-center justify-center mb-4`}>
                  <span className={f.iconColor}>{f.icon}</span>
                </div>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${f.badgeStyle} mb-3 inline-block`}>
                  {f.badge}
                </span>
                <h3 className="font-bold text-slate-800 text-lg mb-2">{f.title}</h3>
                <p className="text-slate-500 text-sm mb-4">{f.desc}</p>
                <Link href={f.link} className="text-sky-500 text-sm font-semibold flex items-center gap-1 hover:gap-2 transition-all">
