...
export default function ChatBot() {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      if (lastMessage.role === "bot" && lastMessageRef.current && containerRef.current) {
        const container = containerRef.current;
        const lastEl = lastMessageRef.current;
        const timer = setTimeout(() => {
          container.scrollTo({
            top: lastEl.offsetTop - 12,
            behavior: "smooth",
          });
        }, 80);
    ...
    <div className="fixed bottom-4 left-4 right-4 sm:bottom-6 sm:right-6 sm:left-auto z-50 flex flex-col items-end pointer-events-none">
      {isOpen && (
        <div className="pointer-events-auto mb-4 w-full sm:w-96 rounded-2xl shadow-2xl bg-white border border-slate-100 flex flex-col overflow-hidden max-h-[calc(100vh-120px)]" 
          ...
          {/* Messages */}
          <div ref={containerRef} className="relative flex-1 overflow-y-auto p-4 flex flex-col gap-3 bg-slate-50">
            {messages.map((msg, i) => (
              <div 
                key={i} 
